...
export default function SettingsPage() {
  ...
  return (
    <div className="min-h-screen bg-surface pb-24">
      <header className="bg-surface-container-lowest px-6 py-4 sticky top-0 z-10 border-b border-outline-variant/10 shadow-sm">
      ...
      <main className="p-6">
        {settingsSections.map((section) => (
          <div key={section.title} className="mb-6">
            <h2 className="text-xs font-bold text-on-surface-variant/70 uppercase tracking-wider mb-3 px-1">{section.title}</h2>
            <div className="bg-surface-container-lowest rounded-2xl overflow-hidden shadow-sm border border-outline-variant/10">
              {section.items.map((item, itemIndex) => (
                <Link
                  key={item.id}
                  href={item.href}
                  className={`flex items-center gap-4 p-4 hover:bg-surface-container-high/50 active:bg-surface-container-high transition-colors ${
        ...
        <div className="bg-surface-container-lowest rounded-2xl overflow-hidden shadow-sm mt-6 border border-outline-variant/10">
          <button
            onClick={handleSignOut}
            disabled={loading}
            className="w-full flex items-center gap-4 p-4 hover:bg-error/10 active:bg-error/15 transition-colors"
          >
            <div className="w-11 h-11 bg-error/10 rounded-xl flex items-center justify-center flex-shrink-0">
              <span className="material-symbols-outlined text-error">logout</span>
            </div>
            <div className="flex-1 text-left">
              <p className="font-bold text-error">{loading ? t.settings.signingOut : t.settings.signOut}</p>
              <p className="text-xs text-on-surface-variant mt-0.5">{t.settings.signOutSub}</p>
            </div>
            <span className="material-symbols-outlined text-on-surface-variant/40">chevron_right</span>
          </button>
        </div>
